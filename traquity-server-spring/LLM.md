...
- **Database password channel**: the packaged Electron spawn hands the H2 file password over the process's stdin rather than its
  environment, marked by `TQ_DB_FILE_PASSWORD_STDIN=true` in the environment.
  ...
  `TQ_DB_FILE_PASSWORD` property ahead of the system environment via `common/startup/StdinPasswordHandover`. The read is one-shot per JVM
  ...
  never touched and `application.yaml`'s `${TQ_DB_FILE_PASSWORD:}` placeholder falls through to the environment variable exactly as before;
