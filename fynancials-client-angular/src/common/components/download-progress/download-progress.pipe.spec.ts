...
type FyDecimalTransform = (value: number, digitsInfo: string) => string;
type FyPercentTransform = (value: number, digitsInfo: string) => string;

describe('DownloadProgressPipe', (): void => {
  let pipe: DownloadProgressPipe;
  let downloadPercentageMock: jest.Mock<DownloadPercentage>;
  let fyDecimalTransformMock: jest.Mock<FyDecimalTransform>;
  let fyPercentTransformMock: jest.Mock<FyPercentTransform>;
  ...
  beforeEach((): void => {
    ...
    // the pipe handed to its collaborators, without duplicating FyDecimalPipe/FyPercentPipe's own formatting logic
    fyDecimalTransformMock = jest.fn((value: number, digitsInfo: string): string => `${value}(${digitsInfo})`);
    fyPercentTransformMock = jest.fn((value: number, digitsInfo: string): string => `${value}(${digitsInfo})%`);

    pipe = new DownloadProgressPipe(
      {transform: fyDecimalTransformMock} as unknown as FyDecimalPipe,
      {transform: fyPercentTransformMock} as unknown as FyPercentPipe
    );
  });
  ...
  it('passes the fraction downloadPercentage computed straight to the percent pipe', (): void => {
    ...
    expect(fyPercentTransformMock).toHaveBeenCalledTimes(1);
    expect(fyPercentTransformMock).toHaveBeenCalledWith(0.75, '1.0-0');
  });
  ...
  it('drops the total size and percentage when the total is unknown', (): void => {
    ...
    expect(fyPercentTransformMock).not.toHaveBeenCalled();
  });
  ...
  it('rounds sizes to whole MiB before formatting, and passes the rate through undivided beyond MiB/s', (): void => {
    ...
    expect(fyDecimalTransformMock.mock.calls).toEqual([[2, '1.0-0'], [1.25, '1.1-1']]);
  });
