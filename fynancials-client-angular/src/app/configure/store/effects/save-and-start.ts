...
export function saveAndStart(signalStore: ReadableSignalStore<SaveAndStartSlice>,
  ...
  return rxMethod<void>(saveAndStartPipe(signalStore, bridge, startupStore, selectedDatabasePath));
}
 ...
 * contributes its own slice of them - #38's Java section adds one there and nothing else here changes.
 *
...
export function saveAndStartPipe(signalStore: ReadableSignalStore<SaveAndStartSlice>,
  ...
  return pipe(
    ...
    map((databasePath: string): ConfigurationChanges => ({databasePath})),
