 ...
 * `FY_DB_FILE_PASSWORD` goes for a different reason: the database password reaches a backend over its stdin, and an
 ...
 * spawn merging `fynancials.config.json`'s own `env` sanitizes after that merge, so a key written into the config is
