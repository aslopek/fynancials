const {traquityConfigSchema} = require('./config-schema.js');

/** @import {TraQuityConfig} from './config-schema.js' */

/**
 * Reads and writes `traquity.config.json`. The file system and the file's location are injected, so this module is
 ...
 * @property {TraQuityConfig} config the parsed configuration, or the default one when it could not be read
 ...
 * @property {(config: TraQuityConfig) => void} save
 */
...
function createConfigFile(options) {
  ...
  function load() {
    ...
    const parsedConfig = traquityConfigSchema.safeParse(contents);
