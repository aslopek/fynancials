...
`fynancials-api` contains the OpenAPI 3.0.3 specs that are the source of truth for the Fynancials HTTP API. It has no runtime code of its
own — it's purely specs plus validation scripts. It's consumed as a codegen input by `fynancials-client-angular` (Angular client) and
`fynancials-server-spring` (Spring server delegates); see the root `LLM.md` for that cross-package workflow.

## Specs

One YAML file per domain, each fully self-contained (no `$ref`s across files — common shapes like `Id` and `Version` schemas are duplicated
...
- `openapi-generator-cli` version is pinned per-package in `openapitools.json` (`generator-cli.version`) and shared via
  ...
  sync with `fynancials-client-angular/openapitools.json` and the plugin version in `fynancials-server-spring/pom.xml`.
